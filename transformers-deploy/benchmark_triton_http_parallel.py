...
def main(call_args):
    ...
    if args.length == 128:
        ...
        text = "testing the multilingual on AWS" #"This live event is great. I will sign-up for Infinity."
    ...
    total_text = num_queries * batch_size
    print_timings(name=f"triton transformers {client_idx}", timings=time_buffer)
    qps = total_text/diff_time
    print(f"Client {client_idx}: {qps:.2f} qps")
    return qps
...
if __name__ == "__main__":
    ...
    parser.add_argument("--model",  default="transformer_onnx_inference")
    parser.add_argument("--batch_size", type=int, default=1)
    parser.add_argument("--num_queries", type=int, default=1_000)
    ...
    with Pool(args.num_clients) as p:
        qps_total = sum(list(p.imap_unordered(main, call_args)))
    
    print(f"Total QPS: {qps_total}")
